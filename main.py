# ==========================================
# Project: מתווך בקליק | Version: 1232-G2
# ==========================================
...
if "step" not in st.session_state:
    st.session_state.update({
        ...
        "show_nav": False, "lesson_txt": "", "current_sub": None
    })

# הצגת כותרת ושם משתמש קבועים (אם מחובר)
if st.session_state.user:
    st.title(f"🏠 מתווך בקליק | 👤 {st.session_state.user}")
else:
...
EXAMS_DATABASE = {
    "test_exam_1": {
        ...
        "questions": [{"q": f"שאלה {i+1}: מהו המלל הנכון?", 
...
if st.session_state.step == "login":
    ...
    if st.button("⬅️ חזרה לנושאים"): st.session_state.step = "study"; st.session_state.lesson_txt = ""; st.rerun()
...
st.markdown(f'<div style="text-align:center; color:grey; font-size:0.7em;">Version: 1232-G2</div>', unsafe_allow_html=True)
