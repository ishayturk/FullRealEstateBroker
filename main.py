...
# ניהול מצב סיידבר: פתוח רק בבחינה
s_state = "expanded" if ("step" in st.session_state 
         and st.session_state.step == "exam_run") else "collapsed"
...
# עיצוב CSS לשחזור הסטריפ והאלמנטים
st.markdown("""
    <style>
    * { direction: rtl; text-align: right; }
    header, #MainMenu, footer { visibility: hidden; }
    .block-container { max-width: 800px !important; margin: auto !important; 
    ...
    .timer-box {
        background: #262730; color: #00ff00; padding: 15px; 
        text-align: center; font-size: 2rem; font-family: monospace;
...
# הסטריפ העליון המקורי
st.markdown(f"""
    <div class="fixed-header">
        <div><span style="font-size: 1.2rem; font-weight: bold;">
        🏠 מתווך בקליק - מערכת בחינות</span></div>
...
if "step" not in st.session_state or st.session_state.step == "instructions":
    ...
    st.write("5. בסיום 90 דקות המבחן יינעל. 6. ציון עובר: 60.")
    st.write("")
...
# דף בחינה
    ...
    with st.sidebar:
        st.markdown(f'<div class="timer-box">{logic.get_timer_display()}</div>', 
                    unsafe_allow_html=True)
        st.write("ניווט:")
        for r in range(0, 25, 4):
            ...
            for i, col in enumerate(cols):
                idx = r + i + 1
                if idx <= 25:
                    is_active = (idx in st.session_state.answers_user 
                                 or idx == st.session_state.current_q)
    ...
    if q_data:
        ...
        choice = st.radio("בחר תשובה:", q_data["options"], 
                          index=st.session_state.answers_user.get(
                              st.session_state.current_q), 
                          key=f"r_{st.session_state.current_q}")
        if choice:
            st.session_state.answers_user[
                st.session_state.current_q] = q_data["options"].index(choice)
...
# דף סיכום
    ...
    for res in results:
        ...
        if not res["is_correct"]:
            st.markdown(f"*תשובה נכונה:* {res['correct_text']}")
        st.write("---")
