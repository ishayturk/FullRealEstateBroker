...
# Version: V210 | Date: 24/02/2026 | 00:35
...
if st.session_state.step == "instructions":
    ...
    with center_col:
        instructions = ["המבחן כולל 25 שאלות.", "זמן מוקצב: 90 דקות.", "מעבר לשאלה הבאה רק לאחר סימון תשובה וטעינת השאלה.", "ניתן לחזור אחורה לשאלות שנחשפו.", "ציון עובר: 60."]
        ...
        if agree and not is_q1_ready:
            st.caption("מכין את השאלה הראשונה...")
    ...
    with col_main:
        ...
        if q:
            ...
            if choice is not None:
                ...
                if st.session_state.current_q == 25: st.session_state.finish_button_visible = True
            st.divider()
            ...
            with b_next:
                if st.session_state.current_q < 25:
                    ...
                    is_next_ready = (st.session_state.current_q + 1) in st.session_state.exam_data
                    if st.button("לשאלה הבאה", disabled=not (is_answered and is_next_ready)):
                        ...
                        if curr <= 23: logic.ensure_question_exists(curr + 2)
                        st.rerun()
